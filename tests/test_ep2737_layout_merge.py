...
def _embed_ids_and_missing_rels(docx_path: Path) -> tuple[int, int]:
    with zipfile.ZipFile(docx_path, "r") as z:
        rels = z.read("word/_rels/document.xml.rels").decode("utf-8")
        doc = z.read("word/document.xml").decode("utf-8")
    rel_ids = set(re.findall(r'Id="(rId\d+)"', rels))
    embed_ids = set(re.findall(r'r:embed="(rId\d+)"', doc))
    return len(embed_ids), len(embed_ids - rel_ids)
...
def test_layout_merge_preserves_generated_image_relationships(tiny_png):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        embeds, missing = _embed_ids_and_missing_rels(output)
        assert embeds >= 1
        assert missing == 0
